...
Input shape (see SKILL.md and references/EXTRACTION.md):
...
FX_TIMEOUT_S = 8
FX_RETRIES = 1  # 1 retry on top of the initial attempt
MEGA_ROUND_THRESHOLD = 100_000_000

CATCHALL_BASE_URL = os.environ.get(
    "CATCHALL_BASE_URL", "https://catchall.newscatcherapi.com"
)
CATCHALL_PAGE_SIZE = 100
CATCHALL_TIMEOUT_S = 30
...
def fetch_fx_rates(needed_currencies):
    """
    Returns (rates_dict, fx_note).

    - On success with non-USD currencies present: rates dict populated and
      a human-readable note for the dashboard footer.
    - On no non-USD currencies: ({"USD": 1.0}, None).
    - On FX failure with mixed currencies: (None, None) → caller enters
      USD-only mode.

    Rates dict maps codes to USD→other (so to convert native→USD: divide).
    """
    ...
    last_err = None
    for _ in range(FX_RETRIES + 1):
        try:
            req = urllib.request.Request(
                FX_ENDPOINT,
                headers={"User-Agent": "vc-pack/2.0"},
            )
            with urllib.request.urlopen(req, timeout=FX_TIMEOUT_S) as resp:
                data = json.loads(resp.read())
            if data.get("result") != "success":
                last_err = f"FX endpoint returned non-success: {data.get('result')}"
                continue
            rates = dict(data.get("rates") or {})
            rates["USD"] = 1.0
            updated = data.get("time_last_update_utc", "unknown")
            note = (f"FX rates: live from open.er-api.com "
                    f"(refreshed {updated}).")
            return rates, note
        except (urllib.error.URLError, OSError, json.JSONDecodeError, KeyError) as e:
            last_err = repr(e)
            continue
    sys.stderr.write(f"vc-pack: FX fetch failed ({last_err}); USD-only mode\n")
    return None, None
...
# =============================================================================
# Input acquisition — three modes
# =============================================================================

def fetch_catchall_pull(job_id: str, api_key: str,
                        base_url: str = CATCHALL_BASE_URL) -> dict:
    """Pull all pages of a CatchAll job over HTTP. Returns a normalized
    pull dict with `candidate_records`, `valid_records`, `all_records`
    (across all pages), and `date_range`.

    Avoids the round-trip through Claude tokens: when the agent has the
    job ID, this script can fetch the data itself in one call instead of
    Claude having to re-emit ~50KB of JSON via the LLM.
    """
    pages = []
    page = 1
    while True:
        url = (f"{base_url}/catchAll/pull/{job_id}"
               f"?page={page}&page_size={CATCHALL_PAGE_SIZE}")
        req = urllib.request.Request(url, headers={
            "x-api-key": api_key,
            "User-Agent": "vc-pack/2.0",
        })
        with urllib.request.urlopen(req, timeout=CATCHALL_TIMEOUT_S) as resp:
            data = json.loads(resp.read())
        pages.append(data)
    ...
    if not pages:
        return {"candidate_records": 0, "valid_records": 0,
                "all_records": [], "date_range": {}}
    first = pages[0]
    all_records = []
    for p in pages:
        all_records.extend(p.get("all_records") or [])
    return {
        "candidate_records": first.get("candidate_records", 0),
        "valid_records": first.get("valid_records", len(all_records)),
        "all_records": all_records,
        "date_range": first.get("date_range") or {},
    }
...
def build_input_data(args: argparse.Namespace) -> dict:
    ...
    Modes (in priority order):
      1. --funding-job-id + --ma-job-id    : direct API pull
      2. --funding-pull   + --ma-pull      : load saved pull files
      3. positional input.json             : pre-assembled combined input
    """
    ma_pending = bool(getattr(args, "ma_pending", False))
    if args.funding_job_id and (args.ma_job_id or ma_pending):
        api_key = args.api_key or os.environ.get("CATCHALL_API_KEY", "")
        if not api_key:
            sys.exit("vc-pack: --funding-job-id requires CATCHALL_API_KEY env "
                     "var or --api-key flag.")
        funding = fetch_catchall_pull(args.funding_job_id, api_key)
        ma = fetch_catchall_pull(args.ma_job_id, api_key) if args.ma_job_id else {}
    elif args.funding_pull and (args.ma_pull or ma_pending):
        ...
        sys.exit(
            ...
            "  --funding-job-id X --ma-job-id Y  (with CATCHALL_API_KEY env)\n"
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Three input modes:\n"
            "  1. --funding-job-id X --ma-job-id Y  (fastest; needs CATCHALL_API_KEY)\n"
            "  2. --funding-pull P1  --ma-pull P2   (saved pulls; auto-unwraps MCP)\n"
            "  3. <input.json>                      (pre-assembled combined input)"
        ),
    )
    parser.add_argument("input", nargs="?", default=None,
                        help="Path to combined input JSON (mode 3).")
    parser.add_argument("--funding-job-id",
                        help="CatchAll funding job ID (triggers direct API pull).")
    parser.add_argument("--ma-job-id",
                        help="CatchAll M&A job ID (triggers direct API pull).")
    ...
    parser.add_argument("--api-key", default="",
                        help="CatchAll API key (else CATCHALL_API_KEY env var).")
