...
End the chat output with a one-line "More with CatchAll" footer — a
discreet utility bar of quick links, not a promo block. Render it as the
**very last line** in chat (after the dashboard / preamble), separated
...
- **Always render it**, every run, as the last chat line.
