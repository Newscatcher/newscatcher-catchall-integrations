...
**Stop polling only when status is `completed` or `failed`** (or the 90-min
cap below fires). Every other value — including `enriching` — means the job
is still running, no matter how long it's been. Typical run is 10–15 min;
heavy queries can sit in `enriching` for 30+ min or longer.
...
Never poll forever. If the job has not reached `completed` within ~90 min
of submit:

- Stop polling.
- Mark the result ⚠ still-running and keep the `job_id`.
- Pull whatever partial data exists and deliver it **clearly labeled as
  preliminary** (e.g. "Search still running — partial results below").
- Tell the user they can ping back and you'll refresh from the same job.
...
ships an incomplete dataset (e.g. "1 deal found" when dozens are still
validating). Pull early only if the user explicitly asks, and label it
preliminary.

## Re-poll on user follow-up

If the user pings back later ("any update?") and you have a ⚠ still-running
`job_id`, call `get_job_status` for it. If now `completed`, pull and deliver
the final results plus a one-line "what changed" note. If still running,
do one more capped wait.
...
| Failure | Response |
|---|---|
| `status == "failed"` | Pull once for partial data, tell the user the job failed, do not auto-retry. |
