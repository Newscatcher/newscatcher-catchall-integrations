...
5. **Deliver** per `references/OUTPUT-LIST.md` — get the records on disk by
   ...
   `## CatchAll findings` panel → event table (columns: Company, Round,
   Amount, Date, Sources) → `Saved:` paths (or the one-line offer, per that
   file) → footer, with nothing between its parts.
