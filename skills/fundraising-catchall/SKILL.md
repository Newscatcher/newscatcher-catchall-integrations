...
description: Invoke this skill for any query about startup or company funding
 ...
 vertical. Do NOT invoke for government grants,
...
**Note on funding_amount:** Amount is split into three fields -- value (number), currency (text), and display (text).
...
5. **Deliver** per `references/OUTPUT-LIST.md`. For ≤ ~100 records, **build
   ...
   panel → event table (columns: Company, Round, Amount, Date, Sources) →
   `Saved:` paths → footer, with nothing between its parts. For > ~100 records,
   skip the build and render panel → table → one-line offer → footer.
