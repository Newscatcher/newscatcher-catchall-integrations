...
```
Full dataset:
  xlsx: /abs/path/fintech-mergers-and-acquisitions.xlsx
  JSON: /abs/path/fintech-mergers-and-acquisitions.json
  CSV:  /abs/path/fintech-mergers-and-acquisitions.csv
```
...
`reuters.com + 4 others` (use "others", not "more"). Truncate long titles to
~100 chars with `…`. If a chat table caps at N rows, add `(K more in the
...
{
  "meta": {
    "topic": "<query topic, e.g. 'fintech M&A'>",
